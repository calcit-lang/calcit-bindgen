...
Bool/Buffer/Number/String、递归同质 List、闭合单态 Option/Result 及 Unit 结果的 core module 打包为可运行 WebAssembly Component。公开、版本化的 Interface IR 和
...
Bool/Buffer/Number/String, recursively homogeneous List, closed monomorphic Option/Result, and Unit-result core modules as runnable WebAssembly Components. Calcit core owns
...
Component generation 当前严格接受 monomorphic Bool/Buffer/Number/String、递归同质 `List<T>`、闭合单态 `Option<T>` / `Result<T,E>` 与 Unit 结果，并产生规范化
...
的 `f64`，从闭合 item schema 递归生成 `list<T>`，并从同一类型树生成 `option<T>` 与 `result<T,E>`；Unit result 省略返回类型，Result 的 Unit 分支使用 WIT 省略 payload 语法。不猜测 Dynamic、异构成员或匿名 variant。只生成严格可表示的 monomorphic subset；Unit field/parameter、generic declaration/application
等失败会包含精确的 definition/declaration type path。CI 使用 Bytecode Alliance `wit-parser`，发布前
同时用 Wasmtime 运行 Bool/Buffer/Number/String、递归 List、Option/Result、Unit、混合签名与 host import smoke，并用 jco
...
currently accepts monomorphic Bool/Buffer/Number/String, recursively homogeneous `List<T>`, closed Option/Result, and Unit-result definitions. It
emits canonical
...
representable subset, derives recursive `list<T>`, `option<T>`, and `result<T,E>` only from closed schemas, and reports precise definition/declaration type paths for
...
recursive-List, Option/Result, Unit, mixed-signature, and host-import smokes in Wasmtime, and transpiles and executes
...
Number maps to current WIT `f64`.
